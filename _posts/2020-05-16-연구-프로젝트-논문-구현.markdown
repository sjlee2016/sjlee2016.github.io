...
===
해당 데이터는 pcap 파일이기 때문에 머신러닝 모델에 쓸 수 있게 csv 파일 포멧으로 바꿔주어야 했습니다. 그러하여 etwork상의 packet을 분석해서 보여주는 무료 오픈 툴인 wireshark의 커멘드를 사용하여 csv 파일을 생성하였습니다. 
```
...
```
CSV 를 생성할때 feature 로 사용될 frame.time_delta, packet size, ack number 와 length 저장하도록 하였습니다.
