...
이를 통해 테이블을 이해하면 (4=KN,DT,RF<,NN)가지 모델은 모두 굉장히 높은 정답률을 가졌다는걸 확인 할 수 있다. 그 중에 제일 낮은 정답률을 가진 것은 Support vector machine with linear kernel(LSVM) 이였는데 이는 데이터가 linear 하게 나눠질 수 없다는 걸 뜻합니다.
