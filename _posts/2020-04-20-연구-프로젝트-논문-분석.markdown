...
이 실험을 재구현을 하게 된다면 제일 걱정되는 부분인, 바로 데이터를 수집하는 단계입니다. 저자도 언급했드시 IoT DoS 트래픽 데이터는 퍼블릭 데이터가 없어 직접 네트워크를 구축하고 트래픽을 만들어서 어택을 하고 데이터 수집해야 합니다.
...
이를 통해 수집한 정상적인 트래픽과 도스 어택의 정보를 합치고 소스 IP 주소, Mac 주소와 패킷 전송 시간을 도용하여 마치 IoT기기가 도스 공격과 정상적인 트래픽을 동시에 생성하는 것 처럼 시뮬레이션을 하였습니다. 이를 통해 491,855 패킷(459,565 악의적인 공격 패킷과 32,290 정상적인 패킷) 을 생성하였습니다. 
...
이 그래프를 보면 normal traffic은 패킷 사이즈가 0~1200 bytes에 다양하게 분포되있지만 attack traffic은 거의 대부분이 100 bytes 보다 작다는걸 확인 할 수 있습니다. 


![alt text](/images/interpack_interval.png "Logo Title Text 1")
###### 이미지 출처: Machine Learning DDoS Detection for consumer Internet Of Things Devices
이 그래프를 보면 attack traffic 은 normal traffic에 비해 패킷들의 interval 을 시간에 대한 1차 미분 한 값과 2차 미분 한 값이 더 빨리 증가한다는걸 볼 수 있습니다. 
...
5가지의 다른 분류 모델을 사용하여 머신러닝 학습을 시켰습니다. 수집한 데이터 85%를 사용하여 트레이닝을 시켰고 나머지 15% 데이터로 학습 결과를 테스트 하였습니다. 결과는 아래와 같습니다. 

분류 모델 성능 평가 지표를 이해하기 위해서는 머신러닝 모델이 틀리는 경우의 수들을 먼저 이해 해봅시다.
...
이 실험에서 False Positive은 실제로 normal 한 traffic을 DoS 어택라고 틀리게 예측하는게 됩니다. 한편, False Negative은 실제 Dos 어택을 normal traffic 이라고 생각하여 탐지하지 못하는게 되겠습니다. 

표에서는 총 4가지의 지표를 사용하여 모델의 성능을 비교합니다. 

1. Precision(정밀도) : True라고 예측한 것들 중에서 실제로 True인 비율 
  true positives/ (true positive + false positive)
2. Recall(재현율) : 실제로 정답이 True 인 것들 중에서 분류기가 True로 예측한 비율
   true positive/(True Positive + False Negatives)
3. F1(조화평균): recall, precision 둘 중 하나가 0에 가깝게 낮을 때 지표에 그것이 균형있게 반영하여 모델의 성능이 좋지 않다는 것을 잘 확인하기 위한 지표. 
   2 * (precision*recall) / (precision+recall)
  
4. Accuracy (정확도) : 전체 예측 건수에서 정답을 맞힌 건수의 비율 
 (true positive + true negative) / (true positive + true negative + false positive + false  negative)

이를 통해 테이블을 이해하면  4가지 (KN,DT,RF,NN) 모델은 모두 굉장히 높은 정답률을 가졌다는걸 확인 할 수 있습니다. 5가지 모델중에 제일 낮은 정답률을 가진 것은 Support vector machine with linear kernel(LSVM) 이였는데 이는 데이터가 선형적으로 분리될 수 없다는 걸 나타냅니다. 

![alt text](/images/gini_score.png "Logo Title Text 1")
###### 이미지 출처: Machine Learning DDoS Detection for consumer Internet Of Things Devices

위 지표는 지니 불순도입니다. 이는 집합에 이질적인 것이 얼마나 섞였는지를 측정하는 지표이며 어떤 집합에서 한 항목을 뽑아 무작위로 라벨을 추정할 때 틀릴 확률을 말합니다. 집합에 있는 항목이 모두 같다면 지니 불순도는 최솟값(0)을 갖게 되며 이 집합은 완전히 순수하다고 할 수 있습니다. 이 지표를 통해 우리는 # destinations, bandwidth 인 stateful feature 들은 Gini score이 매우 낮은걸 알 수 있는데 이는 normal traffic과 attack traffic을 해당 feature을 통해 구분하기 어렵다는걸 뜻합니다. 반대로 packet size와 is_HTTP(protocol 분류) 와 같은 stateless feature들은 높은 gini score 을 가져서 이항 분류를 하는데 굉장히 중요한 특징이라는걸 알 수 있습니다. 
...
하지만 그렇다고 stateful feature이 전혀 도움이 되지 않는걸까요? stateless feature 만을 사용한 모델과 두 가지를 모두 사용한 모델을 비교한 지표입니다. stateful feature을 통해 F1 스코어가 모든 알고리즘에서 0.01~0.05 증가한걸 볼 수 있습니다.


저자는 이 논문은 실제 환경에서 IoT Dos 탐지를 위한 머신 러닝 모델 연구에 동기를 부여할 것이라 말합니다. 또한 앞으로 정확도를 더 높이기 위해 더 많은 양, 그리고 다양한 IoT device를 사용하여 데이터를 수집할 것을 말합니다. 또한 더 다양한 feature을 사용하고 더 복잡한 머신러닝 기술을 사용한다면 더 성능이 높은 파이프라인을 구성할 수 있을것이라 주장합니다.
